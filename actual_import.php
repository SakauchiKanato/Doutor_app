        ...
        if (isset($_POST['confirm_mapping'])) {
            ...
            try {
                ...
                foreach ($csv_data as $index => $row) {
                    ...
                    // 必須項目の取得
                    $date = isset($mapping['log_date']) && $mapping['log_date'] !== '' ? trim($row[$mapping['log_date']]) : '';
                    $item_name = isset($mapping['item_name']) && $mapping['item_name'] !== '' ? trim($row[$mapping['item_name']]) : '';
                    $consumption = isset($mapping['consumption']) && $mapping['consumption'] !== '' ? (int)$row[$mapping['consumption']] : 0;
                    ...
                    $notes = isset($mapping['notes']) && $mapping['notes'] !== '' && $row[$mapping['notes']] !== '' ? trim($row[$mapping['notes']]) : '';
                    
                    // データ検証
                    if (empty($date) || empty($item_name) || $consumption < 0) {
                        $errors[] = "行{$line_num}: 必須項目が不正です（日付={$date}, 商品={$item_name}, 消費量={$consumption}）";
                        $error_count++;
                        continue;
                    }
                    ...
                    if ($existing) {
                        ...
                        // 新規挿入
                        $stmt = $pdo->prepare('
                            INSERT INTO inventory_logs (log_date, item_id, consumption, remaining_stock, notes)
                            VALUES (?, ?, ?, ?, ?)
                        ');
                ...
                $import_message = "<div class='alert alert-success'>✅ インポート完了: 新規{$success_count}件、更新{$update_count}件</div>";
                if ($error_count > 0) {
